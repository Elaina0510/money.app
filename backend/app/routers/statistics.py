...
async def get_summary(
    ...
    result = await statistics_service.get_summary(db, period, start_date, end_date, current_user)
...
async def get_trend(
    ...
    result = await statistics_service.get_trend(db, group_by, start_date, end_date, current_user)
    return success_response(data=result)
...
async def get_compare(
    ...
    result = await statistics_service.get_compare(
        db, start_date_1, end_date_1, start_date_2, end_date_2, current_user
    )
    return success_response(data=result)
