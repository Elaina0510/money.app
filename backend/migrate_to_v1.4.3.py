...
# ── 阶段 B 挂点（M12 续写：budgets 重建 + budget_categories 关联表）──────────
async def migrate_budgets_phase_b(conn: Any, merge_map: dict[int, int]) -> None:
    """阶段 B 挂点：由 v1.4.3 M12 续写（设计 §12.2）。

    约束（D1）：**不得破坏单事务**——本函数与阶段 A 同处 `engine.begin()` 的连接内，
    阶段 A 产出的 `merge_map`（loser→kept）在内存中直接接力：
    `cid' = merge_map.get(old.cid, old.cid)`。阶段 B 失败即整体回滚，阶段 A 不落库。
    """
    del conn, merge_map
    print("  [SKIP] 阶段 B（budgets 命名预算重建）待 v1.4.3 M12 续写")
