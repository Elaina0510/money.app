...
- [x] [M5 - 导入导出改为设置二级页面](m5-import-export-subpage.md) —— 需求五（6）
- [x] [M6 - 二级页面间距疏朗化（三页 + 全局口径）](m6-page-spacing.md) —— 需求六（7）
- [x] [M7 - 设置页账号区用户行头像缩进对齐](m7-account-row-indent.md) —— 需求七（8）
...
| M5 | 前端 | ✅ 已完成 | `9364394` | 新页 /settings/import-export 搬迁（剪切非复制）；四处内联卡用例口径反转 |
| M6 | 前端 | ✅ 已完成 | `ca9a482` | 全局 `.page-card`/`.section-title` 定义已落（P2 窗口闭合）；分类页 scoped margin 静默覆写已修 |
| M7 | 前端 | ✅ 已完成 | `ea7f61a` | .account-user-row 44px 缩进两分支同挂；头像零改动 |
...
- **M9 补登（`e00285a`）**：任务 3.1 断言口径偏离详见下方 M9 条目。
- **M6 补登（`ca9a482`）**：完成报告要点——分类页 scoped `.category-list-item margin:2px` 静默压回全局 4px 已修；「全部标签/全部模板」两处新增文案（任务假定既有分组标题不成立）；v1.4.2 块用例M4-1/M8-1 `.page-card padding 16→20px` 与 HistoryPage 换锚两处按设计 §6.4-2 点名就地改写；轻微差异保留：分类页 density=compact 未改（48px 下限后无视觉差）、标签页无 v-list 2.1 自然 N/A；待人工清单 5.2 已按任务文件原文。
- **M8 补登（`8652bec`）**：完成报告 notes 另含——阶段 A 期 budgets 若挂 loser 分类，FK 检查须待阶段 B 消费 merge_map 后消解（发布两阶段同事务一次跑完，无现实暴露窗口，测试⑦注释登记）；CSV 导入新建分类不设 sort_order（任务 4.3 明示维持现状，代码注释登记）；D9 快速模板派生补 2 条专项测试（前棒真缺口）。
- **M7 完成（`ea7f61a`）**：行号按内容定位（M5 迁出后账号区漂移至 :126-157）；旧 describe「M7 设置页三个管理区块」为 v1.4.2 遗留命名与本模块无关，未动。

- **M5 完成（`9364394`）**：① 口径反转改写 4 处（用例1 摘要入口 3→4、M8-2 头像计数重分配、M8-4 `.entry-avatar` 收紧为 toHaveLength(7)+四行反转零残留、M8-5 区块骨架反转+锚点改），意图均保持、零删除；② **设计偏离 1 处（已裁定接受）**：新页 onMounted 补 `if (!categoriesStore.loaded) fetchCategories()`——直达 URL 时映射弹窗候选缺失的真实缺口，用例M5-8 双向锁定；③ **交接 M14**：`SettingsImportExportPage.vue` 持 1 处 `<v-dialog`（SQL 确认）+CsvMappingDialog×2 引用，收编时实时 grep 会命中；SettingsPage.vue 迁出后已无 `<v-dialog`；M1 的 router :14 meta 未动，M5-7 用例锁定了路由追加位置，router 现处可安全追加状态。
