...
- [ ] [M2 - 主页大卡「总收支」三视图点按切换](m2-dashboard-total-views.md) —— 需求二（2）
- [x] [M3 - 账单页月份条放大 + 选中月滚动居中 + 选中态修复](m3-month-selector.md) —— 需求三（3、4）
- [ ] [M4 - 日期弹窗中文化 + 实时显示 + 选后不关](m4-datepicker-zh-keepopen.md) —— 需求四（5）
...
- [ ] [M10 - 主页/账单页行图标 primary 色系统一](m10-row-icon-primary-unify.md) —— 需求十（11、12）
- [x] [M11 - Bug 修复：竖屏左右滑动误切标签页](m11-swipe-tab-switch-fix.md) —— 需求十一（13）
- [ ] [M12 - 预算模型重构：每月多条命名预算 ★重构+迁移](m12-budget-model-rebuild.md) —— 需求十二（14）
...
### M4
- [ ] 6.2 日历无英文残留；选后不关 + 实时显示；关闭后按最终日期过滤（Network 单次请求）；记一笔同样不即时收起；明暗主题
...
### M12
- [ ] 13.2 三卡独立进度场景复算（含排除模式已用公式）
- [ ] 13.3 旧数据升级逐卡核对；年视图合计 = Σ 预算
- [ ] 13.4 明暗 × 竖/宽
