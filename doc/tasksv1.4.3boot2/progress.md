...
- **⚠️ 登记关注（M3b 上报）→ ✅ 已修（终验后追加 `da2d011`）**：`app/services/export_service.py` budgets 导出段原硬编码 8 列、不含 `dormant`，备份/还原后休眠行会以 `dormant=0`（动态全部）回归。热修已补齐：export CREATE TABLE + INSERT 带 `dormant`（DEFAULT 0）、import `_create_budget_from_values` 读取 dormant（旧备份缺列 → 0 不误置休眠）、新增 2 条往返回归（dormant=1 保真 + 缺列默认 0）。pytest 296 passed / mypy 90 基线 / ruff clean。
